...
| 导出 | 类型 | 说明 |
|------|------|------|
| `queryModelWithStreaming(params)` | `async generator` | 流式查询模型，yield 消息流事件。核心 API 调用入口 |
| `queryModelWithoutStreaming(params)` | `async function` | 非流式查询，等待完整响应后返回 |
| `getMaxOutputTokensForModel(model)` | `function` | 返回指定模型的最大输出 token 数 |
| `getAPIMetadata()` | `function` | 返回 API 元数据信息 |
| `getExtraBodyParams(betaHeaders?)` | `function` | 返回额外 body 参数，用于 Bedrock 兼容性 |
| `queryHaiku(params)` | `async function` | 使用 Haiku 小模型的快捷查询（用于摘要、分类等轻量任务） |

**`queryModelWithStreaming` 核心参数：**
...
| `classifyAPIError(error)` | `function` | 将 API 错误分类为详细字符串类型：`'aborted'`、`'api_timeout'`、`'rate_limit'`、`'transient'`、`'fatal'` 等 |
...
| 导出 | 类型 | 说明 |
|------|------|------|
| `withRetry(getClient, operation, options)` | `async generator` | 带指数退避的请求重试包装器，处理 529/429/连接错误等 |
| `CannotRetryError` | `class` | 无法继续重试时抛出的错误，携带原始错误和重试上下文 |
| `getRetryDelay(attempt, retryAfterHeader?, maxDelayMs?)` | `function` | 计算指定重试次数的退避延迟时间 |
...
| `Utilization` | `type` | 聚合用量数据：5 小时 / 7 天 / Opus / Sonnet / OAuth Apps 各窗口的利用率。包含字段：`five_hour`, `seven_day`, `seven_day_oauth_apps`, `seven_day_opus`, `seven_day_sonnet`（均为 `RateLimit | null`） |
...
| `getSessionLogs(sessionId, url)` | `async function` | 获取指定会话的所有日志条目（2 个参数：sessionId 和 URL） |
| `getTeleportEvents(sessionId, url)` | `async function` | 获取会话的 teleport 事件 |
| `getSessionLogsViaOAuth(sessionId, oauthToken)` | `async function` | 通过 OAuth token 获取会话日志 |
| `clearSession(sessionId)` | `async function` | 清除指定会话的日志 |
| `clearAllSessions()` | `async function` | 清除所有会话的日志 |

### filesApi.ts — 文件上传下载

| 导出 | 类型 | 说明 |
|------|------|------|
| `downloadSessionFiles(files, config, concurrency?)` | `async function` | 批量从 Anthropic Files API 下载文件到本地 |
| `downloadFile(fileId, config)` | `async function` | 下载单个文件，返回 `Buffer` |
| `downloadAndSaveFile(attachment, config)` | `async function` | 下载单个文件并保存到磁盘，返回 `DownloadResult` |
| `uploadFile(filePath, relativePath, config, opts?)` | `async function` | 上传文件到 Files API（4 个参数：filePath, relativePath, config, opts?） |
...
| `isQualifiedForGrove()` | `function` | 检查用户是否有资格使用 Grove |
| `getGroveNoticeConfig()` | `function` | 获取 Grove 通知配置 |
| `calculateShouldShowGrove()` | `function` | 计算是否应显示 Grove 通知 |
| `checkGroveForNonInteractive()` | `function` | 检查非交互模式下的 Grove 状态 |
| `hasClaudeAiMcpEverConnected()` | `function` | 检查 Claude.ai MCP 是否曾连接过 |
...
| `MCPServerConnection` | `union type` | MCP 服务器连接状态联合类型 |
| `ConnectedMCPServer` | `type` | 已连接服务器的摘要信息 |
| `FailedMCPServer` | `type` | 连接失败的服务器 |
| `NeedsAuthMCPServer` | `type` | 需要认证的服务器 |
| `PendingMCPServer` | `type` | 连接待定的服务器 |
| `DisabledMCPServer` | `type` | 已禁用的服务器 |
...
| 导出 | 类型 | 说明 |
|------|------|------|
| `getMcpToolsCommandsAndResources(client, name)` | `async function` | 获取所有已连接 MCP 服务器的工具、命令和资源（2 个参数） |
| `fetchToolsForClient(client, name)` | `async function` | 从单个 MCP 客户端获取工具列表（LRU 缓存） |
| `fetchCommandsForClient(client, name)` | `async function` | 从单个 MCP 客户端获取命令（prompt）列表（LRU 缓存） |
| `fetchResourcesForClient(client, name)` | `async function` | 从单个 MCP 客户端获取资源列表（LRU 缓存） |
...
| `addMcpConfig(name, config, scope)` | `async function` | 添加 MCP 服务器到指定配置范围 |
| `removeMcpConfig(name, scope)` | `async function` | 从指定范围移除 MCP 服务器 |
...
| `hasMcpDiscoveryButNoToken(serverName, serverConfig)` | `function` | 检查服务器是否配置了 OAuth 发现但尚未完成认证（2 个参数） |
...
| `filterMcpPromptsByServer(prompts, serverName)` | `function` | 按服务器筛选 MCP prompts |
...
| `excludeStalePluginClients(clients)` | `function` | 排除过期的插件客户端 |
...
| `hashMcpConfig(config)` | `function` | 计算 MCP 配置哈希 |
| `isToolFromMcpServer(tool, serverName)` | `function` | 判断工具是否来自 MCP 服务器 |
| `isMcpTool(tool)` | `function` | 判断是否为 MCP 工具 |
| `isMcpCommand(command)` | `function` | 判断是否为 MCP 命令 |
| `describeMcpConfigFilePath(path)` | `function` | 描述 MCP 配置文件路径 |
| `getScopeLabel(scope)` | `function` | 获取配置范围标签 |
| `ensureConfigScope(scope)` | `function` | 确保配置范围有效 |
| `ensureTransport(transport)` | `function` | 确保传输协议有效 |
| `parseHeaders(headers)` | `function` | 解析 HTTP 头 |
| `getMcpServerScopeFromToolName(toolName)` | `function` | 从工具名获取 MCP 服务器范围 |
...
| 导出 | 类型 | 说明 |
|------|------|------|
| `fetchClaudeAIMcpConfigsIfEligible()` | `async function` | 从 Claude.ai 获取组织管理的 MCP 服务器配置（会话级缓存，LRU 缓存） |
...
| 导出 | 类型 | 说明 |
|------|------|------|
| `prefetchOfficialMcpUrls()` | `async function` | 预获取官方 MCP 注册表 URL 列表（无显式参数，内部检查环境变量） |
...
| 导出 | 类型 | 说明 |
|------|------|------|
| `initializeAnalyticsSink()` | `function` | 初始化分析 Sink（启动时调用，幂等）。返回 `void` |
...
| `getInitializationStatus()` | `function` | 获取初始化状态（ discriminated union）：`{ status: 'not-started' } \| { status: 'pending' } \| { status: 'success' } \| { status: 'failed'; error: Error }` |
...
| `checkVoiceDependencies()` | `function` | 检查语音依赖是否已安装（平台特定） |
...
## services/voiceStreamSTT.ts — 语音流式识别
...
## services/voiceKeyterms.ts — 语音关键词
...
| `redownloadUserSettings()` | `async function` | 强制重新下载设置（跳过启动缓存，用于 `/reload-plugins`） |
