...
- **`AppState`** — 全局应用状态，包含以下主要字段：
  ...
  - `mainLoopModelForSession: ModelSetting | null` — 当前会话的主循环模型
  ...
  - `isBriefOnly: boolean` — 精简模式
  - `showTeammateMessagePreview: boolean` — 显示队友消息预览
  - `sessionHooks` — 会话钩子状态
  - `todos` — 待办事项状态
  - `elicitation` — 启发状态
  - `authVersion: number` — 认证版本
  - `advisorModel: string | null` — 顾问模型
  - `replBridgeEnabled: boolean` — REPL 桥接启用状态
  - `replBridgeConnected: boolean` — REPL 桥接已连接
  - `replBridgeSessionUrl: string | null` — REPL 桥接会话 URL
  - `replBridgeExplicit: boolean` — REPL 桥接显式模式
  - `replBridgeOutboundOnly: boolean` — REPL 桥接仅出站
  - `replBridgeSessionActive: boolean` — REPL 桥接会话活跃
  - `replBridgeReconnecting: boolean` — REPL 桥接重连中
  - `replBridgeConnectUrl: string | null` — REPL 桥接连接 URL
  - `replBridgeEnvironmentId: string | null` — REPL 桥接环境 ID
  - `replBridgeSessionId: string | null` — REPL 桥接会话 ID
  - `replBridgeError: string | null` — REPL 桥接错误
  - `replBridgeInitialName: string | null` — REPL 桥接初始名称
  - `replBridgePermissionCallbacks` — REPL 桥接权限回调
  - `channelPermissionCallbacks` — 通道权限回调
  - `showRemoteCallout: boolean` — 显示远程呼叫
  - `computerUseMcpState` — computer use MCP 状态
    - `allowedApps`, `grantFlags`, `lastScreenshotDims`, `hiddenDuringTurn`, `selectedDisplayId`, `displayPinnedByModel`, `displayResolvedForApps`
  - `tungstenActiveSession: boolean` — Tungsten 活跃会话
  - `tungstenPanelVisible: boolean` — Tungsten 面板可见性
  - `tungstenLastCapturedTime: number | null` — Tungsten 最后捕获时间
  - `tungstenLastCommand: string | null` — Tungsten 最后命令
  - `tungstenPanelAutoHidden: boolean` — Tungsten 面板自动隐藏
  - `bagelActive: boolean` — Bagel 活跃状态
  - `bagelUrl: string | null` — Bagel URL
  - `bagelPanelVisible: boolean` — Bagel 面板可见性
  - `remoteSessionUrl: string | undefined` — 远程会话 URL
  - `remoteConnectionStatus: 'connecting' | 'connected' | 'reconnecting' | 'disconnected'` — 远程连接状态
  - `remoteBackgroundTaskCount: number` — 远程后台任务数
  - `replContext: REPLContext` — REPL VM 上下文（vmContext, registeredTools, console）
  - `standaloneAgentContext: StandaloneAgentContext` — 独立 Agent 上下文
  - `inbox: InboxState` — 消息收件箱（含 messages, unreadCount, lastProcessedMessageId）
  - `viewSelectionMode: 'none' | 'selecting-agent' | 'viewing-agent'` — 视图选择模式
  - `footerSelection: FooterItem | null` — 底栏选中项
  - `agentNameRegistry: Record<string, string>` — agent 名称注册表
  - `foregroundedTaskId: string | null` — 前台任务 ID
  - `viewingAgentTaskId: string | null` — 查看中的 agent 任务 ID
  - `companionReaction: string | null` — 伴侣反应
  - `companionPetAt: number | null` — 伴侣宠物时间戳
  - `agentDefinitions: AgentDefinition[]` — agent 定义列表
  - `attribution: string | null` — 属性标记
  - `remoteAgentTaskSuggestions: string[]` — 远程 agent 任务建议
  - `skillImprovement: SkillImprovementState | null` — 技能改进状态
  - `pendingPlanVerification: boolean` — 待验证的计划
  - `workerSandboxPermissions: WorkerSandboxPermissions | null` — worker 沙箱权限
  - `pendingWorkerRequest: PendingWorkerRequest | null` — 待处理的 worker 请求
  - `pendingSandboxRequest: PendingSandboxRequest | null` — 待处理的沙箱请求
  - `promptSuggestion: string | null` — 提示建议
  - `promptSuggestionEnabled: boolean` — 提示建议启用状态
  - `spinnerTip?: string` — 旋转提示文本
  - `agent: string | undefined` — 当前 agent 标识
  - `selectedIPAgentIndex: number` — 选中的 IP Agent 索引
  - `coordinatorTaskIndex: number` — 协调任务索引
  - `kairosEnabled: boolean` — Kairos 启用状态
  - `speculationSessionTimeSavedMs: number | null` — 推测会话节省时间
  - `ultraplanLaunching: boolean` — Ultraplan 启动中
  - `ultraplanSessionUrl: string | null` — Ultraplan 会话 URL
  - `ultraplanPendingChoice: string | null` — Ultraplan 待选择
  - `ultraplanLaunchPending: boolean` — Ultraplan 启动待处理
  - `isUltraplanMode: boolean` — 是否为 Ultraplan 模式

- **`SpeculationState`** — 推测执行状态
  - `idle`: `{ status: 'idle' }`
  - `active`: `{ status: 'active'; id: string; abort: () => void; startTime: number; messagesRef; writtenPathsRef; boundary: CompletionBoundary | null; suggestionLength: number; toolUseCount: number; isPipelined: boolean; contextRef: { current: REPLHookContext }; pipelinedSuggestion?: { text: string; promptId: 'user_intent' | 'stated_intent'; generationRequestId: string | null } | null; ... }`
- **`CompletionBoundary`** — 推测完成的边界类型
  - `complete`: `{ type: 'complete'; completedAt: number; outputTokens: number }`
  - `bash`: `{ type: 'bash'; command: string; completedAt: number }`
  - `edit`: `{ type: 'edit'; toolName: string; filePath: string; completedAt: number }`
  - `denied_tool`: `{ type: 'denied_tool'; toolName: string; detail: string; completedAt: number }`
...
- **`hasPermissionsToUseTool(tool, input, context, ...)`** — 检查工具是否有权执行（权限系统主入口）
  - 涉及规则匹配、模式检查、sandbox 判定等
- **`checkPermissions`** — Tool 接口上的方法，工具实现可覆盖以添加工具特定的权限逻辑
  - 注意：`hasPermissionsToUseTool` 是 permissions.ts 的主入口；`checkPermissions` 是 Tool 接口上的方法，工具实现可覆盖
...
模式类型：`'default' | 'plan' | 'acceptEdits' | 'bypassPermissions' | 'dontAsk' | 'auto'`（`auto` 模式受 `TRANSCRIPT_CLASSIFIER` 功能门控）
...
- **`getToolPermissionContext(mode, cwd)`** — Tool 接口上的方法，用于获取初始工具权限上下文（不是 standalone 函数）
...
- **performStartupChecks.tsx** — 启动检查执行
...
| `completionCache.ts` | `setupShellCompletion(theme: ThemeName)`, `regenerateCompletionCache()` | Shell 自动补全脚本生成器 |
...
| `processSlashCommand.tsx` | 处理斜杠命令输入 |
